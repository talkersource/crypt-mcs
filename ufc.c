 ...
 *
 * @(#)ufc.c	1.1 09/23/91, patchlevel 0
 *
...
int pr_bits(unsigned long *a, unsigned long n);
int set_bits(unsigned long v, unsigned long *b);
int init_des();
void setup_salt(char *s);
void mk_keytab(char *key);
...
/* eperm32tab: do 32 bit permutation and E selection
	 ...
	 The first index is the byte number in the 32 bit value to be permuted
		...
		The table is used and generated internally in setup_salt to speed it up
...
#define S_LOOKUP(i,s) s_lookup(i,s)

#endif

/* Setup the unit for a new salt: the sb tables are regenerated
	 from eref, sbox and perm32
...
void setup_salt(s)
	...
	{ unsigned long i,j,bit,sg;
		...
		*/
		...
		for(i=0; i<2; i++)
			{ unsigned long c=ascii_to_bin(s[i]);
				for(j=0; j<6; j++)
					if((c>>j) & 0x1)
    ...
    */
    ...
    for(bit=0; bit<48; bit++)
      { unsigned long mask1,comes_from;
        ...
        comes_from = perm32[disturbed_e[bit]-1]-1;
    ...
    */
    ...
    bzero((char*)sb0,sizeof sb0); bzero((char*)sb1,sizeof sb1);
    bzero((char*)sb2,sizeof sb2); bzero((char*)sb3,sizeof sb3);
